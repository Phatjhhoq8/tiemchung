...
<div style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.02); border: 1px solid #e2e8f0; padding: 30px;">
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
        <h2 style="font-family: 'Outfit', sans-serif; font-size: 18px; font-weight: 700; color: #1e293b; margin: 0;">
            Danh sách Banner
            <span style="font-size: 14px; font-weight: 400; color: #64748b;">({{ $banners->total() }} banner)</span>
        </h2>
        <a href="{{ route('admin.banners.create') }}" class="btn-primary" style="padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 8px;">
            <i data-lucide="plus-circle"></i> Thêm Banner Mới
        </a>
    </div>

    @if($banners->isEmpty())
        <div style="text-align: center; padding: 40px; color: #64748b;">
            <i data-lucide="image" style="width: 48px; height: 48px; margin-bottom: 12px; color: #94a3b8;"></i>
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 14px;">
                <thead>
                    <tr style="border-bottom: 2px solid #cbd5e1; color: #475569;">
                        <th style="padding: 12px 14px; font-weight: 600; width: 60px;">Thứ tự</th>
                        <th style="padding: 12px 14px; font-weight: 600;">Tiêu đề</th>
                        <th style="padding: 12px 14px; font-weight: 600;">Phụ đề</th>
                        <th style="padding: 12px 14px; font-weight: 600;">URL ảnh</th>
                        <th style="padding: 12px 14px; font-weight: 600;">Trạng thái</th>
                ...
                <tbody>
                    @foreach($banners as $banner)
                        <tr style="border-bottom: 1px solid #e2e8f0; color: #334155; transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#f8fafc'" onmouseout="this.style.backgroundColor='transparent'">
                            <td style="padding: 12px 14px; text-align: center; font-weight: 600; color: #64748b;">{{ $banner->sort_order }}</td>
                            <td style="padding: 12px 14px; font-weight: 700; color: #1e293b;">{{ $banner->title }}</td>
                            <td style="padding: 12px 14px; font-size: 13px; color: #64748b;">{{ Str::limit($banner->subtitle, 50) }}</td>
                            <td style="padding: 12px 14px; font-size: 12px; color: #94a3b8; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $banner->image_url }}</td>
                            <td style="padding: 12px 14px;">
                                <span style="padding: 3px 10px; border-radius: 12px; font-size: 11px; font-weight: 600;
                                    @if($banner->is_active) background-color: #def7ec; color: #03543f;
                            ...
                            <td style="padding: 12px 14px; text-align: center;">
                                <div style="display: inline-flex; gap: 8px;">
                                    <a href="{{ route('admin.banners.edit', $banner->id) }}" class="btn-secondary" style="padding: 6px 12px; border-radius: 6px; border: 1px solid #cbd5e1; background:#ffffff; text-decoration: none; font-size: 13px; font-weight: 600; display: inline-flex; align-items: center; gap: 4px; color: #475569;">
